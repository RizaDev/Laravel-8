...
@section('container')
    ...
    <div class="card mb-3">
      ...
      <div style="max-height: 400px; overflow:hidden;">
        <img src="/storage/{{ $posts[0]->image }}" alt="{{ $posts[0]->category->name }} " class="card-img-top" width="1200">
      </div>
      @else
    ...
    <div class="container">
      <div class="row">
        @foreach ($posts->skip(1) as $post)
          <div class="col-lg-4">
            <div class="card">
              <div class="position-absolute px-3 py-2" style="background-color: rgba(0, 0, 0, 0.5)"><a href="/posts?category={{ $post->category->slug }}" class="text-white text-decoration-none">{{ $post->category->name }}
              </div>
              @if ($post->image)
              ...
              <div class="card-body">
                <h3 class="card-title"><a href="/posts/{{ $post->slug }}" class="text-decoration-none text-dark">{{ $post->title }}</h3>
                <p>
